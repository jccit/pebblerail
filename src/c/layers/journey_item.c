...
const int circle_45deg_point = 6;  // approximation of 10sin(0.25π), position on the circumference at a 45 degree angle
const int circle_stroke_width = 3;

void draw_route_line(GContext *ctx, GRect bounds, bool is_highlighted, bool start, bool end, CallingPointState state) {
...
void journey_item_draw(GContext *ctx, const Layer *cell_layer, bool start, bool end, CallingPointEntry *callingPoint) {
  ...
  draw_route_line(ctx, bounds, is_highlighted, start, end, callingPoint->state);
#endif
}
